...
const Header = ({active, onTabSwitch}) => {
    const tabs = ['Travels', 'Started by me', 'Started by others']
    return (
        <View style={{flexDirection: 'row', paddingTop: 25}}>
            {tabs.map((tab, index) => (
                <TouchableOpacity style={{width: '33%'}} onPress={() => onTabSwitch(index)}>
        ...
        <View style={styles.orderContainer}>
            ...
            <View style={globalStyles.flexRowCenter}>
                <Text style={styles.orderTime}>
                    ...
                    <Text> {moment(order.createdAt).format("D MMMM")}</Text>
                </Text>
                <Text style={[styles.orderStatus, styles[statusTitle]]}>{statusTitle}</Text>
            </View>
            {order.status==='Progress' && (
                <View style={[globalStyles.flexRowCenter, {paddingTop: 16}]}>
                    ...
                    <View style={{flexGrow: 1}}>
                        ...
                        <Text style={styles.orderDriverTitle}>Cargo Car</Text>
                    </View>
                    <GradientButton
        ...
        <KeyboardAvoidingScreen>
            ...
            >
                ...
                {filteredOrder.map((order, index) => {
                    return (
                        <TouchableOpacity key={order._id} onPress={() => navigation.navigate('MainTravelOrderDetail', {orderId: order._id})}>
