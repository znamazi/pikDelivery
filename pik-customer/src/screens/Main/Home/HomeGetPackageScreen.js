    ...
    const completeOrder = () => {
        ...
        Api.Server.getTime()
            .then(({success, timestamp, deltaTime}) => {
                if(deltaTime && Math.abs(deltaTime) > 60)
                    throw {message: 'Server time/date does not match your phone time. Set your phone date/time.'}
                ...
                {!!order && <View style={{flexGrow: 1}}>
                    ...
                    <TouchableOpacity onPress={() => navigation.navigate('HomePackageView', {order})}>
                        <View style={[styles.flexRow, styles.packagesItems]}>
                            <Text style={styles.totalPriceTitle}>{order?.packages?.length} packages to pickup</Text>
                    ...
                    <ViewCollapsable collapsed={scheduleType==='schedule' || !nowSchedule}>
                        <View style={globalStyles.inputWrapper}>
                            <Text style={styles.scheduleComment}>Your pickup order will start immediately</Text>
                        </View>
                    </ViewCollapsable>
                    ...
                    <View style={globalStyles.inputWrapper}>
                        {hasNote ? (
                            ...
                            <Text onPress={() => setHasNote(true)} style={globalStyles.link}>+ Add note</Text>
                        )}
                    </View>
